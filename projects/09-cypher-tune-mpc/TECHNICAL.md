...
- Builtin kit: 16 real drum recordings baked into flash as `const int16_t`
  tables (`src/KitSamples.cpp`, generated by `scripts/build-builtin-kit.py`
  at 22050 Hz, peak-normalized to -1 dBFS), memcpy'd into PSRAM at boot by
  `BuiltinKit::loadAll()` — ~437 KB. They are deliberately **not** played in
  place from flash: an NVS write (theme, brightness) briefly disables the
  flash cache, which would fault the audio task mid-block. `SynthKit` still
  exists but now only synthesizes the metronome click.
- SD kits: `/mpc/kits/<name>/pad01.wav … pad16.wav`, 16-bit PCM **mono**,
  ...
  `CYPHER_TUNE_MAX_SAMPLE_FRAMES` — a cap on *source* frames at the file's
  own rate, applied before any resampling, so it is a per-pad PSRAM budget
  rather than a duration. Missing pad files get the
...
- `loop` (list) / `loop <name>` / `loop off` — backing loop from SD
- `loopvol` / `loopvol <0-255>` — backing loop level
- `settings` / `settings open` / `settings close` / `settings idledim`

All of these dispatch. Before 2026-07-31 the shared router's command table was
capped at 12 entries and silently dropped every registration past it, so
everything from `play` onward in this list was dead over Serial — see
`shared/CrowPanelShared/AppConfig.h` (`CROW_SERIAL_MAX_COMMANDS`).
