...
// Calibration math is carried over verbatim from TouchInput so an existing
// per-board CYPHER_KEYS_TOUCH_* override keeps meaning exactly what it did.
int16_t KeysTouch::mapX(int16_t rawX, int16_t rawY) const {
...
int16_t KeysTouch::mapY(int16_t rawX, int16_t rawY) const {
...
void KeysTouch::tick() {
  ...
  uint32_t now = millis();
  if (polledOnce_ &&
      (int32_t)(now - lastPollMs_) < (int32_t)CYPHER_KEYS_TOUCH_POLL_MS) {
    updatePrimary();
    return;
  }
  polledOnce_ = true;
  ...
  for (uint8_t r = 0; r < rawCount; r++) {
    ...
    int8_t nearest = -1;
    int32_t nearestD2 =
        (int32_t)CYPHER_KEYS_TOUCH_MATCH_RADIUS * CYPHER_KEYS_TOUCH_MATCH_RADIUS;
    for (uint8_t i = 0; i < kMaxContacts; i++) {
      Contact &c = contacts_[i];
...
uint8_t KeysTouch::activeCount() const {
  uint8_t count = 0;
  for (uint8_t i = 0; i < kMaxContacts; i++) {
    if (contacts_[i].active) {
      count++;
...
String KeysTouch::diagnostics() const {
