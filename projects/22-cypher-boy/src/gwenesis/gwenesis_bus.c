...
// feature flags, so the whole file is gated here. Undefined evaluates to 0
// in #if, so a build without -DUSE_GENESIS_CORE=1 pays nothing for this.
#if USE_GENESIS_CORE

// CrowPanel patch: RETRO_GO selects `ROM_DATA = buffer` in load_cartridge()
// instead of realloc()+memcpy(). It is the ONLY use of this macro in the whole
// vendored core (verified by grep), and it is the configuration upstream itself
// builds in. Without it, load_cartridge() reallocs the entire ROM onto the
// INTERNAL heap - where only ~150 KB is free once gwenesis's 130 KB of bss is
// linked - gets NULL back, and memcpy()s to it. That is an instant panic and
// reboot the moment a Genesis ROM is opened.
// With it, the 68K reads straight from the PSRAM buffer GenesisCore allocated,
// and ownership stays with the caller (GenesisCore::stop() frees it).
#define RETRO_GO 1
