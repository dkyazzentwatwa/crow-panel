#ifndef CROW_HID_TYPES_H
#define CROW_HID_TYPES_H

#include <Arduino.h>

// Shared HID vocabulary for the CrowHid stack (extracted from project 21 so
// project 05 and project 21 share one transport layer). Deliberately free of any
// project ProjectConfig.h include: these headers compile inside the shared
// library, which never sees a project's config (see CLAUDE.md, the three-layer
// flag rule). Project-specific tables (e.g. MacroPreset, sized by a per-project
// slot count) stay in the project and build on the types here.

// Modifier bitmask, host-agnostic. The transports translate these to the USB
// keyboard key codes (KEY_LEFT_GUI etc). On a Mac, "GUI" is Command.
enum HidMod : uint8_t {
  kModNone = 0,
...
// USBHIDConsumerControl.h) so the transports can pass them straight to press()/
// send(); duplicating them here keeps project tables free of any USB include
...
// Convenience initializers for project macro tables (keep them readable).
#define MACRO_EMPTY \
