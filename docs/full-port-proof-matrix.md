...
| 02 Cypher Vision Cam | `USE_DISPLAY`, `USE_CAMERA_DRIVER`, `USE_WIFI`, `USE_CAM_SD` | portable touch camera: real SC2336 MIPI-CSI (2-lane, 288 Mbps, RAW8 1024x600@30) through the CSI + ISP drivers core 3.3.8 already links, PPA-blitted 1:1 to the DSI panel; hardware-JPEG stills and MJPEG/AVI clips to SD; soft-AP + MJPEG live feed off the onboard C6. Four-tab console (Live/Gallery/Stream/Settings). Replaces the former inspection kiosk, whose camera stub rested on a mistaken impossibility claim — see risk register | Stage 1 (rename + scaffold) compile-ready. Sensor ID read, first CSI frame, viewfinder fps, AE/AWB behaviour in bright and dim light, a still opening on a computer, a clip playing in VLC, and a phone watching `/stream` are all still to be captured |
