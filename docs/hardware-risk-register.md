...
| 2 | ESP32-C6 hosted firmware version vs core 3.3.8 esp_hosted client | P4 Wi-Fi rides the C6 over SDIO; a version mismatch breaks the link at runtime, not compile time | Stage 5 failure path → Elecrow's firmware upgrade guides (paths in the checklist); core bundles `ESP_HostedOTA` |
| 3 | esp_hosted SDIO pin routing on the CrowPanel | Compile can't see whether Elecrow wired the default SDIO slot | Runtime-only; check Elecrow schematic (`Eagle_SCH&PCB/` in their repo) if Stage 5 fails with the C6 firmware already current |
