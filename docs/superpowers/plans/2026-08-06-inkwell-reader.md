...
> **AMENDED during Task 7's quality review (fix(25): permille uses uncompressed sizes, tocTarget accessor, facade coverage; test(25): discriminating permille fixture, chapterSize oversize returns 0):** `chapterSize(i)` above is documented as "compressed entry size (for book %)" -- that was wrong. `InkBook::permille()` (Task 7) weights chapters against DECOMPRESSED reading-position offsets (loadChapter's own byte positions), so returning `stat.m_comp_size` skewed intra-chapter progress by that chapter's own compression ratio: a probe on a barely-compressible chapter froze at ~50% permille for the last ~35% of the chapter (real prose, which compresses harder, froze for roughly the last ~70% -- less visible, same bug). The shipped `EpubBook::chapterSize()` returns `stat.m_uncomp_size` instead -- same `mz_zip_reader_file_stat` call either way, no extra cost -- and returns 0 outright (not a clamped 16MB) past `kMaxEntryBytes`, matching `readEntry()`'s own rejection exactly (a clamp would have an unreadable oversized chapter eat most of a small book's progress bar for content nobody can ever open). `EpubBook.h`'s comment now reads "uncompressed entry size; 0 if it exceeds kMaxEntryBytes".
>
> A first regression test (`testInkBookPermilleEpub`, added in the fix(25) commit) asserted a mid-chapter offset lands strictly between a chapter's start and end permille, but re-review ran that exact test against a deliberately-reverted `m_comp_size` build and it stayed green -- it does not discriminate the bug. Two independent reasons: its fixture (a 64/65-byte hand-written chapter pair, kept for hand-checkable formula arithmetic) is too small and too unique for deflate to compress at all, so compressed and uncompressed sizes barely differ; and its expected values are derived from `raw.chapterSize()` itself, so a reverted build simply recomputes the same "expected" numbers from its own (wrong) unit and the assertion still holds. The follow-up commit (`test(25): discriminating permille fixture, chapterSize oversize returns 0`) added `buildFixtureEpubCompressibleChapter()` (~60 repetitions of one prose paragraph, ~12KB, compressing at a real ratio) and `testInkBookPermilleEpubDiscriminatesCompression()`, which measures its mid/end offsets from `EpubBook::chapterXhtml()`'s decompressed length -- the domain `Block::srcOffset`/the paginator actually work in -- independent of whatever `chapterSize()` currently returns. Manually verified against a temporarily-reverted `m_comp_size` build: the new test's `atMid` saturated to the same value as `atEnd` (765 == 765, both derived from the chapter's ~225-byte compressed size against an ~11.8KB decompressed chapter) instead of landing strictly between `atStart` and `atEnd`; restored and reran green. The original `testInkBookPermilleEpub` block still exists and still passes, but is now documented as a formula/arithmetic pin, not a compression-unit regression guard.
...
> **AMENDED during Task 7's quality review (fix(25): permille uses uncompressed sizes, tocTarget accessor, facade coverage):** the interface block above is what shipped in the first cut of `InkBook.h`, minus one behavioral fix and three additions the review required:
> - `permille()`'s underlying `chapterSizes_` now come from `EpubBook::chapterSize()`'s corrected uncompressed size (see the Task 5 amendment above) -- no interface change, but the numbers it returns for EPUB moved.
> - Added `bool tocTarget(size_t i, size_t &chapter) const` -- `toc()` alone leaks a TOC entry's `spineIndex == -1` (unresolved href) straight to the UI, which would otherwise try to jump to chapter `(size_t)-1`. `tocTarget()` returns false for that case (and for `i` out of range), true with the resolved chapter otherwise; MD's TOC entries always resolve to chapter 0.
> - Added `bool coverImage(std::string &out, std::string &mediaType)` -- a thin passthrough to `epub_.coverImage()` (false, untouched params, for TXT/MD) so Task 13's cover-thumbnail work doesn't need a second `EpubBook` instance/open path of its own just to read the cover.
> - Added a private `uint64_t totalSize_` cache, computed once at `open()` instead of re-summed on every `permille()` call -- the HUD's position scrubber (Task 12) calls `permille()` once per drag sample, so re-summing `chapterSizes_` per call would have been a per-frame O(chapters) cost for no reason.
>
> `docs`/tests: `EpubBook.h`'s `chapterSize()` comment and `InkBook.cpp`'s approximation caveat were slimmed to drop the now-fixed compression-ratio-skew language (see Task 5's amendment); only "sizes are per-chapter byte weights" remains. `test/host_main.cpp` gained `testInkBookTocTargetEpub`/`Markdown`/`Txt`, `testInkBookCoverImage`, `testInkBookCloseResetsState`, and `testInkBookNullEmptyOpenTxt`/`Markdown` (the last two are the only tests exercising `open(Txt, nullptr, 0)`'s empty-buffer guard). A separate follow-up commit (`test(25): discriminating permille fixture, chapterSize oversize returns 0`) then replaced this round's non-discriminating permille regression test with one that actually catches the compressed-vs-uncompressed-size bug -- see the Task 5 amendment above for why the first version didn't and what the replacement does differently.
...
> **AMENDED during execution (commit c2af046):** two claims above didn't survive contact with the actual repo. (1) There is no `SD_MMC.setPins()` call anywhere in this codebase — checked every SD_MMC consumer (projects 08/09/13/15/18/20/21/22) — and `HardwareProfile.h` has no SD-card pin group to copy one from (`HostedSdioPins` is a different SDIO bus entirely, for the onboard ESP32-C6 radio link). Every one of those projects mounts the CrowPanel's card slot with board-default pins via plain `SD_MMC.begin(mountpoint, oneBit)`; `LibraryStore` does the same. (2) `DeskStorage.cpp` itself never calls `SD_MMC.begin()`/`setPins` at all — the real mount owner in project 18 is `DeskSystemServices.cpp`'s `DeskStorageService::mountCard()` (the "don't re-mount a card another subsystem already brought up" guard and the `alreadyMounted`/`SD_MMC.cardType()` check are ported from there, not from `DeskStorage.cpp`). Inkwell is the only SD consumer in this project, so `LibraryStore::begin()` owns the mount outright rather than delegating to a separate service. (3) The book cap shipped at 32, not 64 — `LibraryStore::kMaxBooks`'s comment in `src/LibraryStore.h` has the memory arithmetic (32 fixed `BookEntry` slots is on the order of 2KB even full, negligible against the ~300KB of internal DRAM this board has to share with everything else; 64 wasn't wrong for headroom reasons, it just wasn't the number that shipped).
