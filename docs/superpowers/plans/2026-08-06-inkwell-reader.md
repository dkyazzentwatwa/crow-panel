...
> **AMENDED during Task 7's quality review (fix(25): permille uses uncompressed sizes, tocTarget accessor, facade coverage):** `chapterSize(i)` above is documented as "compressed entry size (for book %)" -- that was wrong. `InkBook::permille()` (Task 7) weights chapters against DECOMPRESSED reading-position offsets (loadChapter's own byte positions), so returning `stat.m_comp_size` skewed intra-chapter progress by that chapter's own compression ratio: a probe on a barely-compressible chapter froze at ~50% permille for the last ~35% of the chapter (real prose, which compresses harder, froze for roughly the last ~70% -- less visible, same bug). The shipped `EpubBook::chapterSize()` returns `stat.m_uncomp_size` instead, clamped to `kMaxEntryBytes` for consistency with `readEntry()`'s own cap -- same `mz_zip_reader_file_stat` call either way, no extra cost. `EpubBook.h`'s comment now reads "uncompressed entry size, clamped to kMaxEntryBytes". `test/host_main.cpp` gained a mid-chapter `permille()` regression test (`testInkBookPermilleEpub`) asserting a mid-chapter offset lands strictly between that chapter's start and end permille, which the old compressed-size bug would have failed by saturating early.
